...
<form class="layui-form" action="" lay-filter="example">
    ...
    <div class="layui-form-item">
        <div class="layui-input-block">
            ...
            <button type="submit" class="layui-btn" lay-submit="" lay-filter="checkinSub">确认提交</button>
...
<script>
    ...
    layui.use(['form', 'layedit', 'layer'], function() {
        ...
        //监听提交
        form.on('submit(checkinSub)', function (data) {
            layer.confirm("是否确认进行入住登记？",function () {
                $.ajax({
                    type: "GET",
                    url: "checkinAction", //提交的网址
                    data: data.field,
                    datatype: JSON, //返回数据的格式
                    success: function (res) {//成功返回之后调用的函数
                        if (res == 1) {
                            layer.confirm("登记成功！！！",function(){
                                window.location = "roomList";
                            });
                        } else {
                            layer.alert("登记失败！请检查内容格式");
                        }
                    },
                    //调用出错执行的函数
                    error: function () {
